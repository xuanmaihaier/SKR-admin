 ...
 * @LastEditors: stride
 * @LastEditTime: 2021-05-04 16:45:08
-->
<template>
  <div class="serve">
    <div class="rList">
      <a-list item-layout="horizontal" :data-source="rList">
        <a-list-item slot="renderItem" slot-scope="item, index" @click="itemClick(item.toAccount,item.conversationID)">
    ...
    <div class="ser_text">
      <div class="contents" ref="contents" v-if="conversationID!=''">
        ...
        <div v-for="(item,index) in Ulist" :class="item.to!=='user1'?'user_bar':'store_bar'">
      ...
      <div class="master" v-else>
        <div class="center">
          <img src="../../assets/img/figthing.gif" height="300px" alt="">
          <p>{{timerNow}},今天请继续加油</p>
        </div>
      </div>
      <a-textarea type="textarea" ref="textarea" class="textarea" placeholder="请尽快回复" v-model="message" @keyup.enter="sendMessage_init" :auto-size="{ minRows: 5, maxRows: 5 }" />
  ...
  methods: {
    ...
    im_login() {
      ...
      let promised = this.tim.login({
        userID: "user1",
        userSig:
          "eJwtzMEKgkAUheF3udtC7rWZqRHa6iIhxDYtDUe5TGPDaCJE756py-Md*D9wy8toNAESiCOE-bK5Nt3ADS-87k2g7ehrW3nPNSQkECVqpNP6mMlzMLNLKWNEXHVg9zdFWhy0QLVVuJ27OVepJmGPmXvIXdt2-VgO07XMnpe0GO-q5XSF1FjDxRm*PxYqMWk_",
      });
      promised
    ...
    itemClick(toAccount, conversationID) {
      ...
      let promise = this.tim.getMessageList({ conversationID, count: 15 });
      promise.then(res => {
      ...
      if (this.login_) {
        ...
        promise
          .then(function (imResponse) {
            ...
            that.message = "";
            let promise = that.tim.getMessageList({ conversationID: that.conversationID, count: 15 });
            promise.then(res => {
              that.Ulist = res.data.messageList
            })
          })
